...
//    }];
    long imageCount = self.urlArray.count;
    CGFloat w = ([UIScreen mainScreen].bounds.size.width-AUTO_MARGIN(40)-60)/3.0;
    CGFloat h = w;

    [self.subviews enumerateObjectsUsingBlock:^(UIButton *btn, NSUInteger idx, BOOL *stop) {
        NSInteger columnIndex = idx % imageCount;
        CGFloat x = columnIndex * (w + HZPhotoGroupImageMargin) + HZPhotoGroupImageMargin;
        CGFloat y = 0;
        btn.frame = CGRectMake(x, y, w, h);
    }];

    self.frame = CGRectMake(0, AUTO_MARGIN(10), [UIScreen mainScreen].bounds.size.width-AUTO_MARGIN(40), h);
}
