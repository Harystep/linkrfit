...
@implementation BLETimerServer

static BLETimerServer* _defaultBTServer = nil;
...
#pragma mark 初始化
+(BLETimerServer*)defaultBLEServer
{
    if (nil == _defaultBTServer) {
        _defaultBTServer = [[BLETimerServer alloc] init];
...
-(void)initBLE
{
    ...
    connectBlock = nil;
    self.temBackValue = @"";
//    NSDictionary * dict = [NSDictionary dictionaryWithObjectsAndKeys:CBCentralManagerOptionRestoreIdentifierKey,CBCentralManagerOptionShowPowerAlertKey, CBCentralManagerScanOptionAllowDuplicatesKey, CBConnectPeripheralOptionNotifyOnDisconnectionKey, nil];
    ...
    if (self.delegate) {
        if([(id)self.delegate respondsToSelector:@selector(didFoundPeripheral)]){
            ...
            [self connect:peripheralInfo withFinishCB:^(CBPeripheral *peripheral, BOOL status, NSError *error) {
                [self stopScan:YES];
                if([(id)weakself.delegate respondsToSelector:@selector(didConnect)]){
                    kUserInfo.status = YES;
...
//    if ([ZCBluthDataTool judgeTimerBluthNameWithContent:peripheral.name]) {
//        if (peripheral.name.length > 0) {
//            [self addPeripheral:peripheral advertisementData:advertisementData RSSI:RSSI];
//        }
//    }
    NSLog(@"%@ -- %@", advertisementData[@"kCBAdvDataLocalName"], peripheral.name);
    NSString *bluthName = checkSafeContent(advertisementData[@"kCBAdvDataLocalName"]);
    if (bluthName.length > 0) {
        if ([ZCBluthDataTool judgeTimerBluthNameWithContent:bluthName]) {
            NSLog(@"发现外设:%@;RSSI:%@", peripheral.name, RSSI);
            if (peripheral.name.length > 0) {
                [self addPeripheral:peripheral advertisementData:advertisementData RSSI:RSSI];
            }
...
- (void)peripheral:(CBPeripheral *)peripheral didDiscoverCharacteristicsForService:(CBService *)service error:(NSError *)error {
    ...
    if (nil == error) {
        for (CBCharacteristic *ch in service.characteristics) {
            if ([[ch.UUID UUIDString] containsString:@"FFE9"]) {
            ...
            if ([[ch.UUID UUIDString] containsString:@"FFE4"]) {
                [peripheral setNotifyValue:YES forCharacteristic:ch];
                [peripheral readValueForCharacteristic:ch];
            }
        }
...
{
    if ([[characteristic.UUID UUIDString] containsString:@"FFE4"]) {
        ...
        if (content.length > 0) {
            if ([content containsString:@"Error"]) {
                self.temBackValue = @"";
                [[NSNotificationCenter defaultCenter] postNotificationName:@"kFSModeBackErrorKey" object:nil];
            } else {
                if ([content containsString:@"no"]) {
                    self.temBackValue = @"";
                    [[NSNotificationCenter defaultCenter] postNotificationName:@"kFSModeBackAgainKey" object:nil];
                } else {
                    if ([content containsString:@"App_Cmd_FS,FreeStyle Open"] || [content isEqualToString:@"App_Cmd_FS,FreeStyle Close"] || [content containsString:@"FreeStyle"]) {
                        self.temBackValue = @"";
                        [[NSNotificationCenter defaultCenter] postNotificationName:@"kFSModeBackKey" object:nil];
                    } else {
                        self.temBackValue = [self.temBackValue stringByAppendingString:content];
                        if ([self.temBackValue containsString:@"App_Cmd_FS,FreeStyle Close"] || [self.temBackValue containsString:@"App_Cmd_FS,FreeStyle Open"]) {
                            NSLog(@"temBackValue-->%@", self.temBackValue);
                            self.temBackValue = @"";
                            [[NSNotificationCenter defaultCenter] postNotificationName:@"kFSModeBackKey" object:nil];
                        } else if ([self.temBackValue containsString:@"App_Cmd_FS,FreeStyle Close"]) {
                            NSLog(@"temBackValue-->%@", self.temBackValue);
                            self.temBackValue = @"";
                            [[NSNotificationCenter defaultCenter] postNotificationName:@"kFSModeBackErrorKey" object:nil];
                        }
