//
//  ZCTimerStopWatchController.m
//  PowerDance
//
//  Created by PC-N121 on 2021/12/9.
//

#import "ZCTimerStopWatchController.h"
#import "ZCTimerOperateView.h"
#import "BLETimerServer.h"

@interface ZCTimerStopWatchController ()

@end

@implementation ZCTimerStopWatchController
...
- (void)setupSubviews {
    UILabel *lb = [self.view createSimpleLabelWithTitle:NSLocalizedString(@"秒表", nil) font:60 bold:YES color:[ZCConfigColor txtColor]];
    [self.view addSubview:lb];
    [lb mas_makeConstraints:^(MASConstraintMaker *make) {
        make.centerX.mas_equalTo(self.view.mas_centerX);
        make.top.mas_equalTo(self.naviView.mas_bottom).offset(AUTO_MARGIN(59));
    }];
    UILabel *subL = [self.view createSimpleLabelWithTitle:NSLocalizedString(@"STOP WATCH", nil) font:25 bold:YES color:[ZCConfigColor txtColor]];
    ...
    [[BLETimerServer defaultBLEServer].selectPeripheral writeValue:[ZCBluthDataTool getSTOPWATCHMode] forCharacteristic:[BLETimerServer defaultBLEServer].selectCharacteristic type:CBCharacteristicWriteWithResponse];
...
- (void)routerWithEventName:(NSString *)eventName {
    if ([eventName integerValue] == 0) {//exit
        ...
        [self startTrainTypeData:[ZCBluthDataTool getSTOPWATCHMode]];
    }
}
