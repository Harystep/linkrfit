//
//  ZCHomeTrainPlanWeightTargetGuideView.m
//  PowerDance
//
...
@interface ZCHomeTrainPlanWeightTargetGuideView ()<rulerDelegate>
...
@implementation ZCHomeTrainPlanWeightTargetGuideView
...
- (void)createSubviews {
    ...
    UILabel *lb = [self createSimpleLabelWithTitle:NSLocalizedString(@"7、请选择你的目标体重", nil) font:14 bold:NO color:[ZCConfigColor point8TxtColor]];
    [self addSubview:lb];
    ...
    self.numL = [self createSimpleLabelWithTitle:NSLocalizedString(@"50KG", nil) font:22 bold:YES color:[ZCConfigColor point8TxtColor]];
...
- (void)backOperate {
    [self routerWithEventName:@"weightTargetBack" userInfo:@{}];
}
...
- (void)btnOperate {
    ...
    [self routerWithEventName:@"weightTargetNext" userInfo:@{@"weightTarget":content}];
    [self saveUserInfoWithData:@{@"targetWeight":checkSafeContent(content)}];
...
- (TTScrollRulerView *)ruler {
    if (!_ruler) {
        ...
        _ruler.lockMin = 20;//最小值
        _ruler.lockMax = 200;//最大值
