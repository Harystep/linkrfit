//
//  ZCHomeTrainPlanTimeGuideView.m
//  PowerDance
//
...
@interface ZCHomeTrainPlanTimeGuideView ()

@property (nonatomic,strong) UIView *selectView;

@end

@implementation ZCHomeTrainPlanTimeGuideView
...
- (void)createSubviews {
    ...
    UILabel *lb = [self createSimpleLabelWithTitle:NSLocalizedString(@"4、你希望每天健身时长", nil) font:14 bold:NO color:[ZCConfigColor point8TxtColor]];
    [self addSubview:lb];
    ...
    CGFloat height = 45;
    CGFloat marginY = 10;
    NSArray *titleArr = @[NSLocalizedString(@"10 ~ 20", nil), NSLocalizedString(@"20 ~ 30", nil), NSLocalizedString(@"30 ~ 40", nil)];
    for (int i = 0; i < titleArr.count; i ++) {
        ...
        [self configureTargetView:targetView title:[NSString stringWithFormat:@"%@%@", titleArr[i], NSLocalizedString(@"分钟", nil)]];
    }
}
...
- (void)selectTargetType:(UITapGestureRecognizer *)tap {
    ...
    [self routerWithEventName:@"timeTrain" userInfo:@{@"timeTrain":@(view.tag)}];
}

- (void)backOperate {
    [self routerWithEventName:@"timeBack" userInfo:@{}];
}
