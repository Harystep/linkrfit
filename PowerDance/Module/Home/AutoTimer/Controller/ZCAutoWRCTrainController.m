...
@interface ZCAutoWRCTrainController ()<UITableViewDelegate, UITableViewDataSource>
...
@implementation ZCAutoWRCTrainController
...
- (void)setupContentSubviews {
    ...
    UILabel *lb = [self.view createSimpleLabelWithTitle:NSLocalizedString(@"WRC（自定义训练模式）", nil) font:20 bold:YES color:[ZCConfigColor txtColor]];
...
- (void)saveOperate {     
    ...
    if (dataStr.length > 0) {
        [HCRouter router:@"AutoWRCMiddle" params:@{@"data":self.dataArr, @"circle":checkSafeContent(self.roundL.text)} viewController:self animated:YES];
    } else {
        [self.view makeToast:NSLocalizedString(@"请完成当前设置", nil) duration:2.0 position:CSToastPositionCenter];
    }
