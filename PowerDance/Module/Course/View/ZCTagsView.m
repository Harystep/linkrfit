...
- (instancetype)init {
    self = [super init];
    if (self) {
        _insets = UIEdgeInsetsMake(2, 20, 2, 20);
        self.font = FONT_SYSTEM(12);
        self.textColor = rgba(0, 0, 0, 0.25);
...
#pragma mark - 视图
@interface ZCTagsView ()

@property (nonatomic, strong) UIView *containerView;

@end

@implementation ZCTagsView
...
- (void)clickLabel:(UITapGestureRecognizer *)tap {
    if (self.clickLabelResule) {
        ...
        if (self.type == 1) {
            self.clickLabelResule([NSString stringWithFormat:@"%tu", lb.tag]);
        } else {
