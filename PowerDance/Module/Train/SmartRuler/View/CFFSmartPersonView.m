...
@implementation CFFSmartPersonView
...
- (void)createSubviews {
    ...
    if ([checkSafeContent(kUserStore.userData[@"sex"]) integerValue] == 0) {
        sexImage = @"body_man";
        chestTop = 158;
        waistTop = 9;
        bigTop = 19;
    ...
    [self addSubviewDataView];
}

- (void)addSubviewDataView {
    [self addSubview:self.dataView];
    [self.dataView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.leading.trailing.mas_equalTo(self);
        make.top.mas_equalTo(self.bgCircleIv.mas_bottom).offset(AUTO_MARGIN(-10));
        make.bottom.mas_equalTo(self.mas_bottom).inset(30);
    }];
//    [self configViewShadowColor:self.dataView];
       
}

- (void)configViewShadowColor:(UIView *)view {
...
- (void)setSelectTargetView:(NSInteger)index {
    ...
    self.dataView.nameL.text = selL.text;
}
...
- (void)setParmsValue:(NSString *)content index:(NSInteger)index {
    ...
    if (self.uploadRulerDataStatusBlock) {
        self.uploadRulerDataStatusBlock(self.parms);
    }
}
...
- (void)setSelectColor:(UIView *)view {
    view.backgroundColor = [ZCConfigColor txtColor];
//    [UIColor colorWithRed:48/255.0 green:203/255.0 blue:165/255.0 alpha:1.0];
}
...
- (void)setRulerData:(NSString *)rulerData {
    ...
    self.selDataL.text = [rulerData substringWithRange:NSMakeRange(0, rulerData.length - 2)];
    self.dataView.dataL.text = rulerData;
    [self setParmsValue:[rulerData substringWithRange:NSMakeRange(0, rulerData.length - 2)] index:self.selectView.tag];
    [self nextSelectTargetView:self.selectView.tag];
    if (rulerData.length > 2) {
        NSString *dataStr = [NSString stringWithFormat:@"0.0%@", [rulerData substringWithRange:NSMakeRange(rulerData.length - 2, 2)]];
        self.dataView.rulerData = dataStr;
    }
}
...
- (void)setStatus:(NSInteger)status {
    _status = status;
    self.dataView.rulerData = [NSString stringWithFormat:@"0.0%@", self.dataView.unit];
    for (UILabel *lb in self.viewArr) {
        lb.text = @"--";
    }
}

- (CFFSmartRulerDataView *)dataView {
    if (!_dataView) {
        _dataView = [[CFFSmartRulerDataView alloc] init];
        __weak typeof(self) weakself = self;
        _dataView.btnClickSaveBlock = ^(NSString * value) {
            weakself.selDataL.text = value;
            [weakself nextSelectTargetView:weakself.selectView.tag];
            [weakself setParmsValue:value index:weakself.selectView.tag];
        };
    }
    return _dataView;
}
