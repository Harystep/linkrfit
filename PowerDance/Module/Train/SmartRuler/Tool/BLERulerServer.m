...
@implementation BLERulerServer

static BLERulerServer *_defaultBTServer = nil;
...
#pragma mark 初始化
+(BLERulerServer*)defaultBLEServer
{
    if (nil == _defaultBTServer) {
        _defaultBTServer = [[BLERulerServer alloc] init];
...
//    NSLog(@"service--->%@", service);
    if (nil == error) {
        for (CBCharacteristic *ch in service.characteristics) {
            if ([[ch.UUID UUIDString] containsString:@"5CB8"]) {
...
- (void)peripheral:(CBPeripheral *)peripheral didUpdateValueForCharacteristic:(CBCharacteristic *)characteristic error:(NSError *)error
{
    ...
    if (data != nil) {
        ...
        for (int i = 0; i < 9; i ++) {
            [dataArr addObject:[NSString stringWithFormat:@"%c", testByte[i]]];
        }
        NSString *backData = [NSString stringWithFormat:@"%@.%@", [self convertContentVlue:dataArr], [self convertContentPoint:dataArr]];
        if ([dataArr[5] isEqualToString:@"S"]) {//确定数据
            if (![self.saveData isEqualToString:backData]) {//发送确定数据通知
                self.saveData = backData;                
                if([(id)self.delegate respondsToSelector:@selector(saveDataResult:unit:)]){
                    [self.delegate saveDataResult:self.saveData unit:dataArr[6]];
                }
            }
        } else {//显示数据
            if (![self.backData isEqualToString:backData]) {//发送数据变化通知
                self.saveData = @"";
                self.backData = backData;
                if([(id)self.delegate respondsToSelector:@selector(backData:unit:)]){
                    [self.delegate backData:self.backData unit:dataArr[6]];
                }
            }
        }
    }
...
- (NSString *)convertContentPoint:(NSArray *)dataArr {
    NSString *content;
    if ([dataArr[6] isEqualToString:@"I"]) {//单位 - 英尺
        content = [NSString stringWithFormat:@"%@%@", dataArr[3], dataArr[4]];
    } else {//单位 - CM
        content = [NSString stringWithFormat:@"%@", dataArr[3]];
    }
    return content;
}

- (NSString *)convertContentVlue:(NSArray *)dataArr {
    NSInteger count = [dataArr[0] integerValue] * 100 + [dataArr[1] integerValue] * 10 + [dataArr[2] integerValue];
