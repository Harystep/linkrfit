//
//  ZCUserHeightGuideCell.m
//  PowerDance
//
//  Created by PC-N121 on 2022/5/11.
//

#import "ZCUserHeightGuideCell.h"
#import "TTScrollRulerView.h"

@interface ZCUserHeightGuideCell ()<rulerDelegate>
...
@implementation ZCUserHeightGuideCell
...
+ (instancetype)userHeightGuideCellWithTableView:(UITableView *)tableView idnexPath:(NSIndexPath *)indexPath {
    ZCUserHeightGuideCell *cell = [tableView dequeueReusableCellWithIdentifier:@"ZCUserHeightGuideCell" forIndexPath:indexPath];
    return cell;
}
...
- (void)createSubviews {
    ...
    UILabel *targetL = [self createSimpleLabelWithTitle:NSLocalizedString(@"你的当前身高是", nil) font:AUTO_MARGIN(14) bold:NO color:rgba(160, 99, 57, 1)];
    ...
    self.numL = [self createSimpleLabelWithTitle:NSLocalizedString(@"162CM", nil) font:30 bold:YES color:rgba(94, 73, 62, 1)];
...
- (void)btnOperate {
    [self routerWithEventName:@"height"];
