...
@interface ZCCustomCourseAddItemCell ()
...
@implementation ZCCustomCourseAddItemCell
...
- (void)createSubviews {
    ...
    self.mouseL = [self createSimpleLabelWithTitle:@"" font:13 bold:YES color:[ZCConfigColor whiteColor]];
    [self.iconIv addSubview:self.mouseL];
...
- (void)deleteOperate {
    NSLog(@"delete·course ---->%tu", self.index);
...
- (void)setActionDic:(NSDictionary *)actionDic {
    ...
    self.mouseL.text = [NSString stringWithFormat:@"%@s", checkSafeContent(actionDic[@"duration"])];
}
