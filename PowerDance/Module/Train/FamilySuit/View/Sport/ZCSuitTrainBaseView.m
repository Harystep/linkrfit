...
@interface ZCSuitTrainBaseView ()

@property (nonatomic,strong) UILabel *titleL;//标题
@property (nonatomic,strong) UILabel *numL;//数量
@property (nonatomic,strong) UILabel *kcalL;//消耗热量
@property (nonatomic,strong) NSArray *imageArr;

@end

@implementation ZCSuitTrainBaseView
...
- (void)createSubviews {
    ...
    self.imageArr = @[kIMAGE(@"suit_num_icon"), kIMAGE(@"suit_num_kcal")];
...
- (void)createSubviewsOnTargetView:(UIView *)targetView type:(NSInteger)type {
    ...
    NSString *title;
    NSString *unit;
    UILabel *contentL = [self createSimpleLabelWithTitle:@"0.0" font:AUTO_MARGIN(30) bold:YES color:[ZCConfigColor txtColor]];
    if (type == 1) {
        unit = @"kcal";
        title = NSLocalizedString(@"卡路里", nil);
        self.kcalL = contentL;
    } else {
...
- (void)setCalorie:(CGFloat)calorie {
    if (calorie > 0.1) {
        self.kcalL.text = [NSString stringWithFormat:@"%.1f", calorie];
    } else {
        self.kcalL.text = [NSString stringWithFormat:@"%.2f", calorie];
    }
}
