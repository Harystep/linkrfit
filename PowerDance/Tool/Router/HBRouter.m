...
static NSString *const HBControllerKey = @"HBControllerKey";
static NSString *const HBRouteKey = @"route";
...
+ (void)loadConfigPlist:(NSString *)plistName{
    plistName = plistName ? plistName : @"HBRouter.plist";
...
+ (UIViewController *)openURL:(NSString *)URL withParams:(NSDictionary *)params{
    ...
    Class controllerClass = routeParams[HBControllerKey];
    UIViewController *viewController = [[controllerClass alloc] init];
    viewController.params = params;
    return viewController;
}

+ (UIViewController *)openURL:(NSString *)URL withParams:(NSDictionary *)params block:(void (^)(id))block {
    NSDictionary *routeParams = [[self sharedInstance] paramsInRoute:URL];
    Class controllerClass = routeParams[HBControllerKey];
    UIViewController *viewController = [[controllerClass alloc] init];

    viewController.params = params;
    viewController.callBackBlock = block;
    return viewController;
}

+ (BOOL)canOpenURL:(NSString *)URL{
    NSDictionary *params = [[self sharedInstance] paramsInRoute:URL];
    if (params[HBControllerKey]) {
        return YES;
    }
    return NO;
}
...
- (NSDictionary *)paramsInRoute:(NSString *)route{
    ...
    params[HBRouteKey] = [self stringFromFilterAppUrlScheme:route];
    
    NSMutableDictionary *subRoutes = self.routes;
    NSArray *pathComponents = [self pathComponentsFromRoute:params[HBRouteKey]];
    ...
    if (class_isMetaClass(object_getClass(class))) {
        if ([class isSubclassOfClass:[UIViewController class]]) {
            params[HBControllerKey] = subRoutes[@"_"];
        } else {
            return nil;
        }
    } else {
...
@implementation UIViewController (HBRouter)
...
- (void)setCallBackBlock:(void (^)(id result))callBackBlock {
    objc_setAssociatedObject(self, &kAssociatedBlockObjectKey, callBackBlock, OBJC_ASSOCIATION_COPY_NONATOMIC);
}

- (HBCallBackBlock)callBackBlock {
